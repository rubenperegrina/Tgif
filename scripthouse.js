...
function check ()
{
    ...
    if (state !== "all") {
        var all = all.filter(stateFilter);
    } else {
        var all = all;
    }
    ...
    for (var i = 0; i<selector.length; i++){
        ...
        if (selector[i].value == "R") {
            rfilter = all.filter(republicans);
        }
        if (selector[i].value == "D") {
            dfilter = all.filter(democrats);
        }
        if (selector[i].value == "I") {
            ifilter = all.filter(independents);
        }
    }
...
function genera_tabla(members) {
    ...
    var sd = document.getElementById("house-data");
...
function genera_dd(members){
    ...
    //Añadimos una option All con value all y la dejamos selected
